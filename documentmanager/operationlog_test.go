...
func TestGetMissingOperations(t *testing.T) {
	...
	assertEqual(t, treedoc2.NewNodeId(A_ID, 1), result[0].Op.Id)
	assertEqual(t, treedoc2.NewNodeId(A_ID, 2), result[1].Op.Id)
	assertEqual(t, treedoc2.NewNodeId(B_ID, 1), result[2].Op.Id)
}
